...
import logging
import random
from utils import Group, Experiment, CheckAssumptionsResult, TestResult
...
class TTestExperiment(Experiment):
    ...
    def perform_test(self) -> TestResult:
        ...
        conclusion = (
            f"Reject the null hypothesis: Significant difference among groups."
            if p_value < self.alpha else
            f"Fail to reject the null hypothesis: No significant difference among groups."
        )
